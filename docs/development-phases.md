...
- **Done**: Transactions is now a real, working screen, not a placeholder. Two new Core pieces back it: `ITransactionApplicationService` (creates a transaction and correctly updates one account for Expense/Income or two accounts for Transfer — the destination-account credit wasn't previously handled anywhere) and `ITransactionBalanceHistoryService` (reconstructs the balance immediately after each historical transaction per account, walking backward from `CurrentBalance` via the existing `ITransactionBalanceService.Reverse`, handling the case where the account is a transfer's destination rather than its source). Both have real Core tests.
  ...
  - Resolve-upcoming widget (`ResolveUpcomingListViewModel`, also composed) projects due/overdue scheduled occurrences (30 days back to 3 days forward) via the existing `IScheduledTransactionProjectionService`, excluding already-paid ones the same way `ForecastService` does. **Mark Paid is real** — it creates the linked transaction and updates balances. **Skip and Delay are not implemented** — deliberately omitted rather than faked, since neither has any backing Core concept yet (no "skipped occurrence" or "delayed occurrence" tracking exists); that's new domain modeling for a future pass, not a UI-only gap.
  - Collapsed filter bar (account, category) filters the list client-side. Select mode toggles per-row checkboxes and enables bulk delete (real). Bulk category-assignment and mass scheduled-assignment from the original spec are not built yet.
  - Category management (rename/merge/delete/recolor/reorder), the manual reconciliation check-in, and statement import are present as header buttons but intentionally disabled/inert placeholders — each is documented elsewhere as its own full-page flow or overlay and wasn't in scope to build blind alongside everything else here.
