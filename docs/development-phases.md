...
- **Done**: the balance chart's point-detail callout now shows on mouse hover (`PointerGestureRecognizer`/`PointerMoved`) rather than requiring a click, for the desktop/mouse case; the existing touch tap-and-drag handling is untouched for a future touch/mobile target. Moving the mouse off the chart reverts to the default (lowest-point) callout rather than hiding it, matching the original always-show-something design.
