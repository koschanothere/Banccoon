...
- **Not yet exposed in the UI**: Settings has no controls yet for choosing the free-to-spend window mode, its rolling-day count, the safety buffer amount, or the major-payment threshold — they currently only take their default values (`RollingDays`/7/0/0) unless changed directly in the database. Wiring these into the Settings page is straightforward now that the Core/schema work is done.
