...
- Steps: read statement → confirm/create account match → *(only when the statement shows bank categories its parser hasn't shown before)* link those bank categories to app categories → review rows.
- **Bank categories** (decided 2026-09-25): a bank's own per-operation categories (Sberbank shows one on every operation) can be linked to app categories. Each bank's list is built from its statements, never written in advance; the linking step lists only never-seen ones, with a category picker (incl. "+ New category") and Skip per row, and anything skipped or left empty is ignored. A learned rule for the recipient still wins over a linked bank category; links only ever set the category, never the type. Settings has a "Bank categories" card above the learned rules for later changes (only banks whose statements were imported; a bank picker when there's more than one). Links are in backups. Review rows show the bank's category in small grey text.
