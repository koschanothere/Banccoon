...
public sealed class SqliteSettingsRepository : SqliteRepositoryBase, ISettingsRepository
{
    ...
    public async Task<AppSettings> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            SELECT DefaultCurrency, DefaultForecastPeriod, ReminderFrequency
            FROM Settings
            WHERE Id = 1;
            """;
    ...
    public async Task SaveAsync(AppSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = """
            INSERT INTO Settings (Id, DefaultCurrency, DefaultForecastPeriod, ReminderFrequency)
            VALUES (1, @DefaultCurrency, @DefaultForecastPeriod, @ReminderFrequency)
